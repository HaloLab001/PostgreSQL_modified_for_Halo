...
#define	stmthist_filenode	13541
	...
	PG_CMD_PUTS("CREATE VIEW public.statement_history AS "
				"SELECT (SELECT datname FROM pg_catalog.pg_database WHERE oid = datid) AS database_name,"
				"       userid::regrole AS username,"
				"       nspid::regnamespace AS schema,"
				"       application_name,"
				"       client_addr,"
				"       client_port,"
				"       query_string,"
				...
				"       finish_time,"
				"       processid,"
				"       (SELECT sum(s.n_soft_parse) FROM pg_catalog.statement_history_internal s WHERE s.query_id = query_id) AS n_soft_parse,"
				"       (SELECT sum(s.n_hard_parse) FROM pg_catalog.statement_history_internal s WHERE s.query_id = query_id) AS n_hard_parse,"
				"       query_plan,"
				"       n_returned_rows,"
				"       n_tuples_fetched,"
				"       n_tuples_returned,"
				...
				"       wait_event,"
				"       is_slow_sql"
				"  FROM pg_catalog.statement_history_internal"
				" WHERE datid = (SELECT oid FROM pg_catalog.pg_database WHERE datname = current_catalog)"
				"    OR current_catalog = 'postgres';\n\n");
